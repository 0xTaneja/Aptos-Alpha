...
class ReferralCommissionManager:
    ...
    def _init_database(self):
        ...
        if cursor.fetchone()[0] == 0:
            # Insert default tiers (amounts in APT)
            tiers = [
                (1, "Bronze", 0, 0.10, "Base commission rate", "#CD7F32"),
                (2, "Silver", 1000, 0.11, "11% commission + priority support", "#C0C0C0"),
                (3, "Gold", 5000, 0.12, "12% commission + VIP dashboard", "#FFD700"),
                (4, "Platinum", 10000, 0.15, "15% commission + custom links + VIP rewards", "#E5E4E2")
            ]
            cursor.executemany(
    ...
    def generate_referral_link(
        ...
        try:
            ...
            referral_url = f"https://t.me/AptosAlphaBot?start=ref_{link_id}"
    ...
    def track_user_volume(self, user_id: str, volume: float) -> Dict:
        ...
        try:
            ...
            # Calculate commission (assuming 0.05% base trading fee on Aptos DEXs)
            base_fee = volume * 0.0005  # 0.05% trading fee
