"""
UserManager class for managing users in the Aptos multi-user architecture
Handles user registration, Aptos agent wallet creation, and status management
"""
import asyncio
import json
import logging
...
class UserManager:
    """
    User management for the Aptos multi-user architecture
    Manages user registration, Aptos agent wallets, and user states
    """
    
    def __init__(self, vault_manager=None, client=None, node_url=None):
        """Initialize the UserManager for Aptos"""
        ...
        logger.info("UserManager initialized with Aptos node URL: %s", self.node_url)
    
    def set_vault_manager(self, vault_manager):
        """Set the vault manager after initialization"""
        self.vault_manager = vault_manager
    
    async def register_user(self, telegram_id: int, aptos_address: str,
                           telegram_username: str = None) -> Dict:
        """
        Register new user with their Aptos address
        
        Args:
            telegram_id: Telegram user ID
            aptos_address: User's Aptos address
        ...
        try:
            ...
            if not self._validate_aptos_address_format(aptos_address):
                return {
                    "status": "error",
                    "message": "Invalid Aptos address format. Address must start with 0x and have 66 characters."
                }
            
            # Normalize address format
            aptos_address = aptos_address.lower()
            
            # Check if address exists on Aptos 
            try:
                # Try to fetch account info to see if address exists
                account_info = self.client.account(aptos_address)
                if not account_info:
                    return {
                        "status": "error",
                        "message": "Could not verify Aptos address. Make sure it's a valid account."
                    }
            except Exception as e:
                logger.error(f"Error checking address on Aptos: {e}")
                return {
                    "status": "error",
                    "message": f"Error verifying Aptos address: {str(e)}"
                }
            
            # Check if user is already registered
            # Create user if not exists or update with Aptos address
            user = await bot_db.add_user(telegram_id, telegram_username)
            
            # Update user with Aptos address if needed
            if not user.get("aptos_main_address"):
    ...
    async def create_agent_wallet(self, telegram_id: int, main_address: str = None) -> Dict:
        ...
        try:
            ...
            if not main_address:
                ...
                main_address = user.get("aptos_main_address")
                
            if not main_address:
                return {
                    "status": "error",
                    "message": "No Aptos address registered. Please provide your address first."
                }
            ...
            agent_account = AptosAccount.generate()
        ...
        try:
            ...
            if agent_address:
                try:
                    ...
                    account_info = self.client.account(agent_address)
                    balance = float(account_info.get("coin", {}).get("value", 0)) / 100000000  # Convert from octas to APT
    ...
    async def create_trading_session(self, telegram_id: int) -> Optional[Dict]:
        ...
        try:
            ...
            # Create session
            user_account = AptosAccount.load_key(private_key)
    ...
    def _validate_aptos_address_format(self, address: str) -> bool:
        ...
        # Basic format check - Aptos addresses are 64 characters (66 with 0x prefix)
        address_pattern = re.compile(r'^0x[a-fA-F0-9]{64}$')
        return bool(address_pattern.match(address))
    
    def _generate_private_key(self) -> str:
        """
        Generate a new Aptos private key
    ...
    async def add_agent_wallet(self, telegram_id: int, agent_address: str,
        ...
        try:
            ...
            async with bot_db.conn.cursor() as cursor:
                ...
                await cursor.execute(
                    """
                    SELECT id, telegram_id, aptos_main_address, 
