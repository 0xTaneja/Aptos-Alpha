...
class AgentWalletManager:
    """
    Manages user agent wallets for Aptos trading without requiring private key exposure
    ...
    def __init__(self, db_path: str = "agent_wallets.db", 
                 node_url: str = None, 
                 main_wallet: Optional[AptosAccount] = None,
                 main_client: Optional[RestClient] = None):
        """
        Initialize the Agent Wallet Manager for Aptos
        
        Args:
            db_path: Path to the SQLite database file
            node_url: Aptos node URL (defaults to mainnet if None)
            main_wallet: Pre-configured main wallet (optional)
            main_client: Pre-configured main client (optional)
        """
        ...
        logger.info(f"AgentWalletManager initialized with DB path: {db_path}, Node URL: {self.node_url}")
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            if self.main_wallet is None or self.main_client is None:
    ...
    async def _initialize_main_wallet(self) -> bool:
        ...
        try:
            ...
            account, client = example_utils.setup_aptos(self.node_url)
            
            # Store the client and account for future use
            self.main_client = client
            self.main_account = account
            
            # ✅ SECURITY FIX: Use actual address from setup, not hardcoded
            self.main_address = str(account.address())  # Use the actual address from setup
            
            # Get and store the main wallet (Account object)
            # We need access to the main wallet to create agent wallets
            if account:
                self.main_wallet = account
                logger.info(f"Main wallet initialized with address: {self.main_address}")
                
                # Log main account balance
                try:
                    account_info = client.account(self.main_address)
                    balance = float(account_info.get("coin", {}).get("value", 0)) / 100000000  # Convert from octas to APT
                    logger.info(f"Main account balance: {balance:.4f} APT")
                except Exception as e:
                    logger.error(f"Error checking main account balance: {e}")
                
                return True
            else:
                logger.error("Account not available, cannot create agent wallets")
                return False
    ...
    async def create_agent_wallet(self, user_id: int, username: str = None, main_address: str = None) -> Dict:
        """
        Create a new agent wallet for a Telegram user's Aptos account
        Uses Aptos account generation and delegation
        Enhanced with duplicate prevention and comprehensive validation
        
        Args:
            user_id: Telegram user ID
            username: Telegram username (optional)
            main_address: User's main Aptos address (required)
        ...
        try:
                ...
                async with aiosqlite.connect(self.db_path) as db:
                    ...
                    ''', (main_address.lower(),)) as cursor:
                        ...
                        if existing_main and existing_main[0] != user_id:
                            ...
                            return {
                                "status": "error",
                                "message": "This Aptos address is already associated with another user"
                            }
            ...
            if not self.main_wallet or not self.main_client:
                return {
                    "status": "error", 
    ...
    def _encrypt_private_key(self, private_key: str) -> str:
        ...
        salt = b'aptos_salt'  # In production, use a secure random salt
        kdf = PBKDF2HMAC(
