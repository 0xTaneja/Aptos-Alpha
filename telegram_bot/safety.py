...
class SafetyManager:
    """
    Comprehensive Aptos safety and risk management system:
    - Daily/weekly APT loss limits
    ...
    def __init__(self, wallet_manager, database=None, config=None):
        ...
        self.default_limits = {
            LimitType.DAILY_LOSS: 50.0,  # 50 APT daily loss limit
            LimitType.WEEKLY_LOSS: 200.0,  # 200 APT weekly loss limit
            LimitType.POSITION_SIZE: 100.0,  # 100 APT max position size
            LimitType.MAX_LEVERAGE: 10.0,  # 10x max leverage
            LimitType.TOTAL_EXPOSURE: 500.0  # 500 APT total exposure limit
        }
...
**Recommended Limits:**
• Daily Loss: 50 APT (conservative) / 100 APT (moderate) / 200 APT (aggressive)
• Weekly Loss: 200 APT (conservative) / 500 APT (moderate) / 1000 APT (aggressive)
...
• Contact support if needed
    ...
    async def generate_safety_report(self, update: Update, context: CallbackContext):
        ...
        report_msg = f"""
📋 **Aptos Safety & Risk Management Report**
*Generated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}*

**🎯 Current Risk Exposure:**
• Total Exposure: {current_exposure:,.2f} APT
• Daily P&L: {daily_pnl:+,.2f} APT
• Weekly P&L: {weekly_pnl:+,.2f} APT
...
{await self._generate_safety_recommendations(user_id)}
    ...
    async def check_risk_limits(self, user_id: int, trade_data: Dict) -> Tuple[bool, List[str]]:
        ...
        if position_limit and position_limit.enabled:
            ...
            if trade_size > position_limit.value:
                violations.append(f"Position size {trade_size:,.2f} APT exceeds limit {position_limit.value:,.2f} APT")
        ...
        if daily_limit and daily_limit.enabled:
            ...
            if abs(potential_loss) > daily_limit.value:
                violations.append(f"Potential daily loss {abs(potential_loss):,.2f} APT exceeds limit {daily_limit.value:,.2f} APT")
        ...
        if exposure_limit and exposure_limit.enabled:
            ...
            if total_exposure > exposure_limit.value:
                violations.append(f"Total exposure {total_exposure:,.2f} APT exceeds limit {exposure_limit.value:,.2f} APT")
    ...
    async def _format_user_limits(self, user_id: int) -> str:
        ...
        for limit_type, default_value in self.default_limits.items():
            ...
            if limit:
                ...
                limits_text += f"• {limit_type.value.replace('_', ' ').title()}: {status} {limit.value:,.2f} APT{breach_info}\n"
            else:
    ...
    async def _calculate_daily_pnl(self, user_id: int) -> float:
        ...
        try:
            ...
            return -12.55  # Example daily loss in APT
        except Exception as e:
    ...
    async def _calculate_weekly_pnl(self, user_id: int) -> float:
        ...
        try:
            # Mock calculation - replace with real implementation
            return 45.02  # Example weekly profit in APT
        except Exception as e:
