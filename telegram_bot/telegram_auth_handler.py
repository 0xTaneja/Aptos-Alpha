"""
Secure authentication handler for Aptos Telegram bot users
"""
import logging
import time
import asyncio
...
class TelegramAuthHandler:
    """Handles secure user authentication for Aptos Telegram bot using agent wallets"""
    
    def __init__(self, user_sessions: Dict[int, Dict[str, Any]], node_url=None, 
        ...
        self.address_verifier = AddressVerificationManager(self.node_url)
    ...
    async def handle_connect_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        await update.message.reply_text(
            "🔐 **Secure Aptos Agent Wallet System**\n\n"
            "Our bot uses a secure Aptos agent wallet system that doesn't require your private key.\n\n"
            "To get started, use the `/create_agent` command to set up your Aptos agent wallet.",
            parse_mode='Markdown',
            reply_markup=InlineKeyboardMarkup([[
    ...
    async def handle_create_agent_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> Optional[int]:
        ...
        if user_id not in user_data:
            await update.message.reply_text(
                ...
                "Please register your Aptos address first using `/start` or `/register_address`.\n\n"
        ...
        user_address = user_data[user_id]['aptos_address']
    ...
    async def _start_agent_creation_with_address(self, update: Update, context: ContextTypes.DEFAULT_TYPE, user_address: str):
        ...
        creating_message = await update.message.reply_text(
            f"🔄 **Creating Aptos Agent Wallet**\n\n"
            f"**Your registered address:** `{user_address[:8]}...{user_address[-6:]}`\n\n"
            f"Creating secure Aptos agent wallet for this address...\n"
        ...
        if result["status"] == "success":
            ...
            approval_message = (
                f"✅ **Aptos Agent Wallet Created Successfully!**\n\n"
                f"**Your Details:**\n"
                ...
                f"**Step 1:** Visit [Aptos Explorer](https://explorer.aptoslabs.com)\n"
            ...
            await creating_message.edit_text(
                ...
                reply_markup=InlineKeyboardMarkup([
                    ...
                    [InlineKeyboardButton("Contact Support", url="https://t.me/aptos_support")]
                ])
            )

    async def handle_address_input(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        """Handle user providing their Aptos address with verification"""
        user_id = update.effective_user.id
        ...
        if verification_result["status"] == "success":
            ...
            await update.message.reply_text(
                ...
                f"Please provide a valid Aptos address.",
                parse_mode='Markdown'
            )
            ...
            if result["status"] == "success":
                ...
                if "fund" in error_message.lower() or "deposit" in error_message.lower():
                    ...
                    await creating_message.edit_text(
                        ...
                        f"**Please check:**\n"
                        f"• Is this your correct Aptos address?\n"
                        f"• Do you have an active account on Aptos?\n"
                    ...
                    await creating_message.edit_text(
                        ...
                        f"Error: {error_message}\n\n"
                        f"**Troubleshooting:**\n"
                        f"• Ensure your Aptos address is correct\n"
    ...
    async def handle_agent_status_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        if not wallet_info:
            ...
            await update.effective_message.reply_text(
                ...
                "• Access to approve the agent in Aptos app\n\n"
                "Ready to start?",
                parse_mode='Markdown',
                reply_markup=InlineKeyboardMarkup([
                    [InlineKeyboardButton("✅ Create Agent Wallet", callback_data=f"create_agent_session_{user_id}")],
    ...
    def _build_status_message(self, wallet_info: Dict, wallet_status: Dict, approval_result: Dict) -> str:
        ...
        if not approved:
            message += (
                ...
                f"**How to approve:**\n"
                f"1. Visit [Aptos Explorer](https://explorer.aptoslabs.com)\n"
    ...
    async def handle_agent_creation_callback(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        await query.edit_message_text(
            "🤖 **Welcome to the Aptos Trading Bot Setup!**\n\n"
            "Do you have an existing Aptos account?",
            parse_mode='Markdown',
            reply_markup=InlineKeyboardMarkup([
        ...
        try:
            ...
            if user_state == APPROVAL_WAITING:
                status_message += (
                    ...
                    "Your agent wallet needs to be approved in your Aptos account settings.\n\n"
                    "Please visit [Aptos Explorer](https://explorer.aptoslabs.com) and approve this agent in your account settings."
                )
                buttons = [
                ...
                status_message += (
                    ...
                    "To fund your account, send APT to your main Aptos address shown above.\n"
                    "Minimum recommended amount: 1 APT\n\n"
    ...
    async def _establish_user_session(self, user_id: int, username: str, wallet_info: Dict):
        ...
        """
        try:
            # Create Aptos client for the session
            ...
            logger.info(f"Established Aptos session for user {user_id} with existing agent wallet")
    ...
    async def handle_has_account_callback(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        """Handle user confirming they have an Aptos account"""
        query = update.callback_query
        ...
        # Ask for their Aptos address
        address_prompt = (
            "📝 **Enter Your Aptos Account Address**\n\n"
            "Please enter your Aptos account address (starting with '0x').\n\n"
    ...
    async def handle_no_account_callback(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        """Handle user confirming they don't have an Aptos account"""
        query = update.callback_query
        user_id = query.from_user.id
        ...
        no_account_message = (
            "❗ **Aptos Account Required**\n\n"
            "You need to create an Aptos account first before using this bot.\n\n"
            "Please visit [Aptos Explorer](https://explorer.aptoslabs.com) to create your account, then come back and use `/create_agent` again.\n\n"
            "If you need help creating your account, visit the [Aptos documentation](https://aptos.dev/guides/getting-started)."
        )
