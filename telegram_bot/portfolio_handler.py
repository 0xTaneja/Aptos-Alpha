...
class PortfolioHandler:
    ...
    async def display_portfolio(self, update: Update, context: CallbackContext) -> None:
        """Display the user's Aptos agent wallet portfolio."""
        ...
        if not user_wallet:
            await message_object.reply_text(
                "❌ No Aptos agent wallet found. Use /agent to create one.",
                parse_mode='Markdown'
            )
            return
        ...
        portfolio_msg = f"""
📊 **Your Aptos Agent Portfolio**
Agent Address: `{user_wallet['address']}`

**Account Value:** {portfolio_data['account_value']:.2f} APT
**Available Balance:** {portfolio_data['available_balance']:.2f} APT
**Unrealized PnL:** {portfolio_data['unrealized_pnl']:.2f} APT
...
**Positions: {len(portfolio_data['positions'])}**
        ...
        if portfolio_data['positions']:
            for pos in portfolio_data['positions']:
                ...
                portfolio_msg += f"\n• {pos['coin']}: {side} {abs(pos['size'])} @ {pos['entry_price']:.4f} APT"
                portfolio_msg += f"\n  Unrealized PnL: {pos['unrealized_pnl']:.2f} APT"
        else:
            portfolio_msg += "\n• No open positions."
        ...
        if portfolio_data['recent_trades']:
            for trade in portfolio_data['recent_trades'][:10]: # Show up to 10 most recent
                ...
                portfolio_msg += f"\n• {trade_time} {trade['coin']}: {trade['side']} {trade['size']} @ {trade['price']:.2f} APT (Fee: {trade['fee']:.4f} APT)"
        else:
            portfolio_msg += "\n• No recent trades."
        ...
        keyboard = [
            ...
            [InlineKeyboardButton("← Back to Aptos Wallet Status", callback_data="refresh_wallet_status")]
        ]
    ...
    async def display_pnl_tracking(self, update: Update, context: CallbackContext):
        await update.message.reply_text("📈 Aptos P&L tracking feature coming soon!")

    # Placeholder for performance analytics
    async def display_performance_analytics(self, update: Update, context: CallbackContext):
        await update.message.reply_text("📉 Aptos performance analytics feature coming soon!")
