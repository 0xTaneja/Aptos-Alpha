"""
Aptos Trading Bot Compliance and legal framework
Handles terms of service, risk warnings, and regulatory compliance for Aptos trading
"""
import logging
...
class ComplianceManager:
    """
    Manages Aptos trading compliance, risk warnings, and legal requirements
    """
    ...
    def __init__(self, config: Dict):
        ...
        self.is_testnet = config.get("aptos", {}).get("network", "mainnet") == "testnet"
        self.environment = "TESTNET" if self.is_testnet else "MAINNET"
        
        logger.info(f"Aptos ComplianceManager initialized for {self.environment}")
...
This bot provides automated trading services on Aptos DEX protocols using agent wallets.
...
• You must be legally able to trade cryptocurrencies
...
Last updated: {datetime.now().strftime('%Y-%m-%d')}
    ...
    def get_risk_warning(self) -> str:
        ...
        env_warning = ""
        if self.is_testnet:
            env_warning = "⚠️ **APTOS TESTNET ENVIRONMENT** - This is for testing only with test funds.\n\n"
        else:
            env_warning = "🚨 **APTOS MAINNET ENVIRONMENT** - This involves real money and real risks.\n\n"
...
**Warning issued: {datetime.now().strftime('%Y-%m-%d %H:%M:%S UTC')}**
    ...
    async def handle_terms_acceptance(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        env_note = ""
        if self.is_testnet:
            env_note = "\n\n🧪 **APTOS TESTNET MODE**: You are using test funds. No real money is at risk."
        else:
            env_note = "\n\n💰 **APTOS MAINNET MODE**: You are using real funds. All risks apply."
