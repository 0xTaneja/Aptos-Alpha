"""
Aptos Trading Bot Admin Panel
Provides comprehensive admin controls and oversight for Aptos trading operations
"""
import asyncio
import json
import logging
...
class AdminPanel:
    """
    Comprehensive admin panel for Aptos trading bot management
    """
    ...
    def __init__(self, config: Dict):
        ...
        logger.info(f"Aptos AdminPanel initialized with {len(self.admin_users)} admin users")
    ...
    async def handle_admin_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        await update.message.reply_text(
            f"🛡️ **Aptos Trading Bot Admin Panel** ({admin_type})\n\n"
            f"Welcome, {update.effective_user.first_name}!\n"
            f"Select an option below to manage the Aptos trading bot:",
            parse_mode='Markdown',
        ...
        try:
            ...
            try:
                async with bot_db.conn.cursor() as cursor:
                    ...
                    await cursor.execute("SELECT COUNT(*) FROM users WHERE aptos_wallet_address IS NOT NULL")
                    result = await cursor.fetchone()
    ...
    async def handle_user_management(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        try:
                ...
                async with bot_db.conn.cursor() as cursor:
                    await cursor.execute('''
                        SELECT telegram_id, telegram_username, status, created_at,
                               aptos_wallet_address, last_active
                        FROM users
            ...
            for user in recent_users[:5]:  # Show top 5
                status_emoji = "✅" if user['has_aptos_wallet'] else "⚠️"
                user_list += (
                    f"{status_emoji} {user['username']} (ID: {user['telegram_id']})\n"
                    f"   Status: {user['status']}\n"
                    f"   Aptos Wallet: {'Yes' if user['has_aptos_wallet'] else 'No'}\n"
    ...
    async def handle_emergency_actions(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        await query.edit_message_text(
            "🚨 **Aptos Trading Bot Emergency Actions**\n\n"
            "⚠️ **WARNING:** These actions affect all users or Aptos trading operations.\n"
