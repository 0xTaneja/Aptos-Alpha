"""
Telegram Bot Handler for Aptos Airdrop Farming
...
# Import your airdrop farmer
from strategies.airdrop import AptosAirdropFarmer

class TelegramAptosHandler:
    def __init__(self, bot_instance):
        self.bot = bot_instance
        self.logger = logging.getLogger(__name__)
        # Optionally, keep a per-user farming task registry
        self.farming_tasks = {}

    def register_handlers(self, application):
        """Register all Aptos-related handlers"""
        application.add_handler(CommandHandler("aptos", self.aptos_command))
        application.add_handler(CommandHandler("airdrop_status", self.airdrop_status_command))
        application.add_handler(CallbackQueryHandler(self.handle_aptos_callbacks, pattern="^aptos_"))

    async def aptos_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """Main Aptos command handler"""
        try:
            user_id = update.effective_user.id
            status = await self._get_airdrop_status(user_id)
            keyboard = [
                [
                    InlineKeyboardButton("🌱 Start Daily Farming", callback_data="aptos_start_farming"),
                    InlineKeyboardButton("📊 View Progress", callback_data="aptos_view_progress")
                ],
                [
                    InlineKeyboardButton("⚙️ Configure Settings", callback_data="aptos_settings"),
                    InlineKeyboardButton("📈 Analytics", callback_data="aptos_analytics")
                ],
                [
                    InlineKeyboardButton("🎯 Manual Activities", callback_data="aptos_manual"),
                    InlineKeyboardButton("ℹ️ Airdrop Info", callback_data="aptos_info")
                ]
            ]
            message = self._format_aptos_overview(status)
            ...
            self.logger.error(f"Aptos command error: {e}")
            await update.effective_message.reply_text(
                "❌ Error accessing Aptos data. Please try again.",
                parse_mode='Markdown'
            )
    ...
    async def airdrop_status_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            keyboard = [
                [InlineKeyboardButton("🔄 Refresh", callback_data="aptos_refresh_status")],
                [InlineKeyboardButton("📊 Weekly Report", callback_data="aptos_weekly_report")]
            ]
            await update.effective_message.reply_text(
                message,
                reply_markup=InlineKeyboardMarkup(keyboard),
    ...
    async def handle_aptos_callbacks(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """Handle all Aptos callback queries"""
        query = update.callback_query
        await query.answer()
        callback_data = query.data
        user_id = update.effective_user.id
        try:
            if callback_data == "aptos_start_farming":
                await self._handle_start_farming(query, user_id)
            elif callback_data == "aptos_view_progress":
                await self._handle_view_progress(query, user_id)
            elif callback_data == "aptos_settings":
                await self._handle_settings(query, user_id)
            elif callback_data == "aptos_analytics":
                await self._handle_analytics(query, user_id)
            elif callback_data == "aptos_manual":
                await self._handle_manual_activities(query, user_id)
            elif callback_data == "aptos_info":
                await self._handle_airdrop_info(query, user_id)
            elif callback_data == "aptos_refresh_status":
                await self._handle_refresh_status(query, user_id)
            elif callback_data == "aptos_weekly_report":
    ...
    async def _handle_start_farming(self, query, user_id: int):
        ...
        try:
            # Prevent concurrent farming for same user
            if self.farming_tasks.get(user_id) and not self.farming_tasks[user_id].done():
                ...
                return
            await query.edit_message_text(
                "🚀 **Starting Aptos Airdrop Farming...**\n\n"
                "⏳ Initializing farming activities...",
                parse_mode='Markdown'
            )
            # Start farming in background
            task = asyncio.create_task(self._execute_farming(user_id, query))
            self.farming_tasks[user_id] = task
        except Exception as e:
            self.logger.error(f"Start farming error: {e}")
            await query.edit_message_text("❌ Error starting farming. Please try again.")

    async def _execute_farming(self, user_id: int, query=None):
        """Execute farming for user using AptosAirdropFarmer"""
        try:
            # Get user Aptos client and account from bot
            aptos_client = await self.bot.wallet_manager.get_user_aptos_client(user_id)
            aptos_account = await self.bot.wallet_manager.get_user_aptos_account(user_id)
            if not aptos_client or not aptos_account:
                from aptos_sdk.async_client import RestClient
                from aptos_sdk.account import Account
                aptos_client = RestClient("https://fullnode.mainnet.aptoslabs.com/v1")
            config = self.bot.main_config.get('aptos_farming', {})
            farmer = AptosAirdropFarmer(aptos_client, aptos_account, config)
            result = await farmer.execute_daily_farming()
            if query:
                if result.get('status') == 'success':
                    completion_rate = result.get('completion_rate', 0)
                    daily_tx = result.get('daily_transactions', 0)
                    target_tx = result.get('target_transactions', 15)
                    message = (
                        f"✅ **Daily Farming Complete!**\n\n"
                        f"📊 **Results:**\n"
                        f"• Transactions: {daily_tx}/{target_tx}\n"
                        f"• Completion: {completion_rate:.1f}%\n"
                        f"• Airdrop Score: {result.get('airdrop_score', 0):.0f}\n\n"
                        f"⏰ Next farming in 24 hours"
                    )
                    keyboard = [[InlineKeyboardButton("📊 View Details", callback_data="aptos_view_progress")]]
                    await query.edit_message_text(
                        message,
                        reply_markup=InlineKeyboardMarkup(keyboard),
                        parse_mode='Markdown'
            ...
            self.logger.error(f"Error in execute_farming: {e}")
            if query:
                await query.edit_message_text("❌ Error running farming. Please try again.")
    ...
    async def _handle_view_progress(self, query, user_id: int):
        ...
        try:
            ...
            keyboard = [
                [
                    InlineKeyboardButton("🔄 Refresh", callback_data="aptos_view_progress"),
                    InlineKeyboardButton("📈 Charts", callback_data="aptos_analytics")
                ],
                [InlineKeyboardButton("⬅️ Back", callback_data="aptos_main")]
            ]
            await query.edit_message_text(
                message,
                reply_markup=InlineKeyboardMarkup(keyboard),
    ...
    async def _handle_settings(self, query, user_id: int):
        ...
        try:
            ...
            message = (
                f"⚙️ **Aptos Farming Settings**\n\n"
                f"🎯 **Current Configuration:**\n"
                f"• Daily TX Target: {current_settings.get('daily_tx_target', 15)}\n"
                f"• Min Trade Size: {current_settings.get('min_trade_size', 5)} APT\n"
                f"• Max Trade Size: {current_settings.get('max_trade_size', 50)} APT\n"
                f"• Auto Farming: {'✅' if current_settings.get('auto_farming', False) else '❌'}\n\n"
                f"📝 Use buttons below to modify settings:"
            )
            keyboard = [
                [
                    InlineKeyboardButton("🎯 Change TX Target", callback_data="aptos_set_tx_target"),
                    InlineKeyboardButton("💰 Set Trade Sizes", callback_data="aptos_set_trade_sizes")
                ],
                [
                    InlineKeyboardButton("🤖 Toggle Auto Farming", callback_data="aptos_toggle_auto"),
                    InlineKeyboardButton("⏰ Set Schedule", callback_data="aptos_set_schedule")
                ],
                [InlineKeyboardButton("⬅️ Back", callback_data="aptos_main")]
            ]
            await query.edit_message_text(
    ...
    async def _handle_analytics(self, query, user_id: int):
        ...
        try:
            ...
            message = (
                ...
                f"• Total Volume: {analytics.get('total_volume', 0):,.2f} APT\n\n"
                f"🎯 **Airdrop Metrics:**\n"
            ...
            for day_data in daily_data[-7:]:
                date = day_data.get('date', 'Unknown')
                tx_count = day_data.get('transactions', 0)
                message += f"• {date}: {tx_count} TX\n"
            keyboard = [
                [InlineKeyboardButton("📊 Export Report", callback_data="aptos_export_report")],
                [InlineKeyboardButton("⬅️ Back", callback_data="aptos_main")]
            ]
            await query.edit_message_text(
                message,
    ...
    # --- Formatting and placeholder methods below ---

    def _format_aptos_overview(self, status: Dict) -> str:
        emoji_status = "🟢" if status.get('farming_active', False) else "🔴"
        return (
            f"🌟 **Aptos Airdrop Farming** {emoji_status}\n\n"
            f"🎯 **Today's Progress:**\n"
            ...
            f"• Est. APT Allocation: {status.get('estimated_allocation', 'TBD')}\n\n"
    ...
    def _format_detailed_status(self, status: Dict) -> str:
        return (
            ...
            f"• DEX Swaps: {status.get('dex_swaps', 0)}\n"
            f"• Aptos Interactions: {status.get('aptos_interactions', 0)}\n"
            f"• Vault Cycles: {status.get('vault_cycles', 0)}\n\n"
            f"💰 **Volume Metrics:**\n"
            f"• Daily Volume: {status.get('daily_volume', 0):,.2f} APT\n"
            f"• Weekly Volume: {status.get('weekly_volume', 0):,.2f} APT\n"
            f"• Total Volume: {status.get('total_volume', 0):,.2f} APT\n\n"
    ...
    # --- Placeholder methods for data retrieval (replace with real DB integration as needed) ---

    async def _get_airdrop_status(self, user_id: int) -> Dict:
        # Replace with DB or farming state
        return {
    ...
    async def _get_farming_progress(self, user_id: int) -> Dict:
        return {
            'activities': {
                ...
                'dex_swaps': {'completed': True, 'transactions': 3, 'target': 3},
                'aptos_interactions': {'completed': False, 'transactions': 2, 'target': 4},
    ...
    async def _handle_manual_activities(self, query, user_id: int):
        await query.edit_message_text("Manual activities coming soon.")

    async def _handle_airdrop_info(self, query, user_id: int):
        await query.edit_message_text("Airdrop info coming soon.")

    async def _handle_refresh_status(self, query, user_id: int):
        await self.aptos_command(query, None)

    async def _handle_weekly_report(self, query, user_id: int):
        await query.edit_message_text("Weekly report coming soon.")
