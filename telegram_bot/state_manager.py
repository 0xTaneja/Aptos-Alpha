"""
State manager for Aptos Trading Bot user flows
Handles user state transitions and corresponding UI for Aptos trading
"""
import logging
import asyncio
import time
...
class StateManager:
    """
    Manages user flow states and UI transitions for Aptos Trading Bot
    Handles all state-specific logic for the Aptos Telegram bot
    """
    ...
    async def get_user_state(self, user_id: int) -> str:
        ...
        try:
            ...
            if not user_data or not user_data.get("aptos_main_address"):
                return UserState.UNREGISTERED
    ...
    async def handle_start(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if user_state == UserState.UNREGISTERED:
            await self.request_aptos_address(update, context)
    ...
    async def request_aptos_address(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """Ask the user for their Aptos address"""
        user_id = update.effective_user.id
        
        # Set user input state to expect an address
        self.user_input_state[user_id] = "awaiting_aptos_address"
        
        welcome_msg = (
            "🎉 **Welcome to Aptos Alpha Bot!**\n\n"
            "Please provide your Aptos account address:"
        )
        
        keyboard = [
            [InlineKeyboardButton("What's Aptos?", url="https://aptoslabs.com")]
        ]
    ...
    async def show_approval_instructions(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        approval_message = (
            "✅ **Aptos Agent created!**\n\n"
            f"Please approve this agent wallet on Aptos:\n"
            f"[Open Aptos Explorer](https://explorer.aptoslabs.com/account/{agent_address})\n\n"
    ...
    async def show_funding_instructions(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        funding_msg = (
            ...
            f"Network: **Aptos Mainnet**\n"
            f"Token: **APT**\n"
            f"Min Recommended: **1 APT**\n\n"
    ...
    async def show_strategy_selection(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        strategy_msg = (
            f"✅ **Funded with {balance:.2f} APT!**\n\n"
            f"Choose a strategy:"
        )
        
        keyboard = [
            [InlineKeyboardButton("🌟 Aptos DeFi", callback_data="strategy_aptos_defi")],
    ...
    async def show_main_menu(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        main_msg = (
            "🤖 **Aptos Alpha Bot**\n\n"
            f"**Balance:** {balance:,.2f} APT\n"
        )
        
        # Add active strategy info if available
        if active_strategy:
            strategy_name = active_strategy.get("name", "Unknown")
            emoji = "🌟" if strategy_name == "aptos_defi" else "📊" if strategy_name == "grid" else "💰"
    ...
    async def handle_text_input(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if expected_input == "awaiting_aptos_address":
    ...
    async def _handle_address_input(self, update: Update, context: ContextTypes.DEFAULT_TYPE, text: str):
        """Handle Aptos address input"""
        user_id = update.effective_user.id
        
        # Clear expected input state
        self.user_input_state.pop(user_id, None)
        
        # Basic address validation (starts with 0x, has 64 characters)
        if not text.startswith("0x") or len(text) != 66:
            await update.message.reply_text(
                "❌ **Invalid Aptos Address**\n\n"
                "Please provide a valid Aptos address starting with '0x' and containing 66 characters.\n\n"
        ...
        self.temp_data[user_id]['aptos_address'] = text
        
        # Show loading message
        loading_message = await update.message.reply_text(
            "🔄 Creating Aptos agent wallet for your account...",
            parse_mode='Markdown'
        )
            ...
            if self.wallet_manager:
                ...
                if wallet_result['status'] in ['success', 'exists']:
                    ...
                    await loading_message.edit_text(
                        f"❌ **Error Creating Aptos Agent Wallet**\n\n"
                ...
                await loading_message.edit_text(
                    "❌ **Error Creating Aptos Agent Wallet**\n\n"
                    "Aptos wallet system not available. Please try again later.",
                    parse_mode='Markdown'
                )
                return
        else:
    ...
    async def handle_strategy_selection(self, update: Update, context: ContextTypes.DEFAULT_TYPE, strategy_type: str):
        ...
        if strategy_type == "aptos_defi":
            await self._configure_aptos_defi_strategy(update, context)
    ...
    async def _configure_aptos_defi_strategy(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """Configure Aptos DeFi strategy"""
        query = update.callback_query
        user_id = query.from_user.id
        ...
        self.strategy_configs[user_id]["aptos_defi"] = {
        ...
        config_msg = (
            "🌟 **Aptos DeFi Mode Configuration**\n\n"
            f"Balance: {balance:.2f} APT\n\n"
            f"Max allocation per opportunity: {recommended_allocation:.2f} APT\n\n"
            "This strategy automatically participates in Aptos DeFi opportunities with your specified allocation."
        )
        
        keyboard = [
            [InlineKeyboardButton(f"Set Max: {recommended_allocation:.2f} APT", callback_data="confirm_aptos_defi_default")],
            [InlineKeyboardButton("Customize Allocation", callback_data="customize_aptos_defi_allocation")],
    ...
    async def _configure_grid_strategy(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        self.strategy_configs[user_id]["grid"] = {
            ...
            "allocation_value": min(balance * 0.5, balance - 1)  # Leave at least 1 APT as buffer
        }
        ...
        config_msg = (
            ...
            "• Grid Levels: 10\n"
            "• Grid Width: 4%\n"
            f"• Allocation: {self.strategy_configs[user_id]['grid']['allocation_value']:.2f} APT\n\n"
    ...
    async def _configure_profit_strategy(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        self.strategy_configs[user_id]["profit"] = {
            ...
            "allocation_value": min(balance * 0.9, balance - 1),  # Leave at least 1 APT as buffer
            "maker_rebate": True,
            "funding_rate": True
        }
        ...
        config_msg = (
            ...
            f"Balance: {balance:.2f} APT\n\n"
            "Default settings:\n"
            "• Trading Pairs: APT, USDC, WETH\n"
            "• Strategies: Maker Rebate Mining, Funding Rate Arbitrage\n"
            f"• Allocation: {self.strategy_configs[user_id]['profit']['allocation_value']:.2f} APT\n\n"
    ...
    async def activate_aptos_defi_strategy(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """Activate Aptos DeFi strategy with configured settings"""
        query = update.callback_query
        user_id = query.from_user.id
        
        # Get configuration
        config = self.strategy_configs[user_id].get("aptos_defi", {})
        ...
        if self.trading_engine:
            try:
                ...
                logger.error(f"Error starting Aptos DeFi strategy for user {user_id}: {e}")
        
        confirmation_msg = (
            "🌟 **Aptos DeFi Mode Activated**\n\n"
            f"Max allocation: {config['max_allocation']:.2f} APT per opportunity\n\n"
            "I'll notify you when new DeFi opportunities are detected and automatically participate according to your settings.\n\n"
    ...
    async def _simulate_aptos_defi_opportunity(self, user_id: int):
        """Simulate an Aptos DeFi opportunity notification (for demonstration)"""
        try:
            # Wait 15 seconds
            ...
            launch_msg = (
                "🚀 **New Aptos DeFi opportunity detected: APTOS_TOKEN**\n\n"
                f"Auto-investing {allocation:.2f} APT..."
            )
            ...
            confirmation_msg = (
                ...
                f"Price: {buy_price:.2f} APT per token\n"
                f"Total: {allocation:.2f} APT"
            )
            ...
            update_msg = (
                "📈 **APTOS_TOKEN Update**\n\n"
                f"Current price: {new_price:.2f} APT (+{pnl_pct:.0f}%)\n"
                f"💰 Unrealized P&L: +{pnl_value:.2f} APT"
            )
            ...
            logger.error(f"Error in Aptos DeFi simulation for user {user_id}: {e}")
    ...
    async def handle_callback(self, update: Update, context: CallbackContext):
        ...
        elif data == "confirm_aptos_defi_default":
            await self.activate_aptos_defi_strategy(update, context)
    ...
    async def _simulate_price_refresh(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        new_price = 1.45
        buy_price = 0.50
        allocation = self.strategy_configs[user_id].get("aptos_defi", {}).get("max_allocation", 100)
        pnl_pct = ((new_price / buy_price) - 1) * 100
        pnl_value = allocation * (new_price / buy_price - 1)
    ...
    async def _show_approval_help(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        keyboard = [
            [InlineKeyboardButton("Go to Aptos Explorer", url="https://explorer.aptoslabs.com")],
    ...
    async def _explain_agent_wallet(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        explanation = (
            ...
            "An agent wallet is a secure way to trade on Aptos without exposing your private keys.\n\n"
            "**How it works:**\n"
            "1. We create an agent wallet linked to your main account\n"
            "2. You approve the agent in your Aptos settings\n"
    ...
    async def show_agent_status(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if user_state == UserState.PENDING_APPROVAL:
            status_msg += (
                ...
                "Please approve your agent wallet in the Aptos app to continue."
            )
            keyboard = [
